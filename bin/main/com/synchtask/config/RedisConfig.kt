 ...
 * **Redis Configuration**
 *
 * - Establishes a secure connection with Redis.
 * - Enables Pub/Sub messaging for real-time notifications.
 * - Uses Lettuce as the Redis driver for better performance.
 */
@Configuration
class RedisConfig(
     ...
     *
     * - Supports connection pooling for better efficiency.
     * - Configures authentication with Redis if a password is provided.
     */
    @Bean
    ...
    /**
     * **Right configuration of RedisTemplate**
     * - Uses **StringRedisSerializer** for keys
     * - Uses "Jackson2JsonRedisSerializer" to serialize "NotificationRedisDTO" correctly.
     */
    @Bean
    ...
    /**
     * **Configures a Redis Message Listener Container**
     *
     * - Dynamically registers Pub/Sub listeners.
     */
    @Bean
    ...
    /**
     * **Maps incoming Redis messages to subscriber methods**
     */
    @Bean
