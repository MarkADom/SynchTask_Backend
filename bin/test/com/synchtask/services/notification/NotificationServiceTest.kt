...
class NotificationServiceTest {
    ...
    fun `should send notification`() {
        ...
        every { storageService.storeNotification(user.email, "New Task", NotificationType.TASK_UPDATE, null) } returns notification
        every { webSocketService.sendNotification(user.email, any()) } just runs

        notificationService.sendNotification(user.email, "New Task", NotificationType.TASK_UPDATE)
    ...
    fun `should get unread notifications`() {
        ...
        val expectedResponse = NotificationMapper.fromRedisDTO(redisNotification)

        every { storageService.getCachedNotifications("user@example.com") } returns listOf(redisNotification)

        val result: List<NotificationResponseDTO> = notificationService.getUnreadNotifications("user@example.com")
