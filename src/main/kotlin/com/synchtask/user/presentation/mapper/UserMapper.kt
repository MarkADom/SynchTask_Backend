...
object UserMapper {
    fun toResponseDTO(user: User): UserResponseDTO = UserResponseDTO(
    ...
    fun toResponseDTOList(users: List<User>): List<UserResponseDTO> = users.map(::toResponseDTO)

    fun toOptionDTO(user: User): UserOptionDTO = UserOptionDTO(
        id = requireId(user.id, "User"),
        name = user.name,
        profilePictureUrl = user.profilePictureUrl
    )

    fun toPublicDTO(user: User): UserPublicDTO = UserPublicDTO(
        id = requireId(user.id, "User"),
        name = user.name,
        profilePictureUrl = user.profilePictureUrl
    )
